...
- **MCP testing (Inspector, env vars, HTTP vs stdio):** [packages/mcp/README.md](packages/mcp/README.md).
- **Mock MCP / Inspector (stdio):** `cd packages/mcp && pnpm run inspector` (alias for `inspector:stdio`, sets `FOUNDRY_USE_MOCK`).
...
| `FOUNDRY_API_TOKEN` | Bearer token for Foundry (required when mock is off). Server-side only. |
| `MCP_HTTP_API_KEY` | If set, `/mcp` requires `Authorization: Bearer <key>`. `/health` is not protected. |
